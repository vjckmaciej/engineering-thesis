...
import com.engineeringthesis.userservice.service.PatientService;
...
public class PatientControllerImpl implements CrudController<PatientDTO> {
    ...
    public ResponseEntity<CrudResponse> add(@RequestBody PatientDTO patientDTO) {
        ...
        return ResponseEntity.ok(new CrudResponse(patientId, "Patient added to database!"));
    }

    @Override
    @RequestMapping(path = "/{patientId}", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
